...
async function FILEopen(filename){
	...
	test_out_group2 = Object.fromEntries(
		Object.entries(test_out_group).map(([key, value]) => [key,Object.groupBy(value, (b) => b[8].replace(/[ _]|CARD|CD|[^\x00-\x7E]/g, ''))])
	);
}
...
function FuncDL(){
	...
	console.log(test_MFG.map(a => a ? a.length < 1 ? a : a.flatMap(b => b) : ""));
